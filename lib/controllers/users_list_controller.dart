...
class UsersListController extends GetxController {
  ...
  void _loadRelationships() {
    ...
    //update relationships when any of the streams change
  ...
  void _updateAllRelationshipsStatus() {
    ...
    for (var user in _users) {
      if (user.id != currentUserId) {
        final status = _calculateUserRelationshipoStatus(user.id);
        _userRelationships[user.id] = status;
      }
    }
  }

  UserRelationshipStatus _calculateUserRelationshipoStatus(String userId) {
    final currentUserId = _authController.user?.uid;
    ...
    try {
      ...
      if (currentUserId != null) {
        ...
        if (request != null) {
          ...
          Get.snackbar(
            'Success',
            'Friend Request from ${user.displayName} accepted',
          );
        }
      }
    } catch (e) {
